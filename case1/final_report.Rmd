...
Although hispanics are technically of European ancestry, it may be worth maintaining them as their own covariate. The granular subsets may, however, be less informative. Investigating the covariate directly, we note the majority reported as not hispanic.
...
As such, we will consider a transformation to turn hispanics into a binary covariate.

Certain counties appear to have significant deviation from the mean. Since county is a proxy for socioeconomic status, it would be worthwhile to link counties through census data to useful socioeconomic indicator such as median income. This would reduce the number of covariates significantly. Unfortunately, we did not have time to do this. So we will choose to ignore the CORES variable for the purposes of this analysis.
...
Note in most cases we observe an initial drop from zero followed by wild fluctuations as the number of cigarettes smoked per day increases. The vast majority of mothers are non smokers throughout each phase of pregnancy.
 ...
 - `r paste0(round(mean(birth_data$CIGPN == 0), 3) * 100, '%')` of women don't smoke during the before pregancy.
